...
def _payload(root: Path, relative_paths: list[str]) -> dict:
    ...
    payload = {
        ...
        "p6_execution_config_sha256": "p6-execution",
        "p5_base_transfer_manifest_sha256": "base",
        "p5_base_transfer_manifest_file_sha256": "base-file",
...
def test_p6_delta_manifest_verifies_exact_files_and_hashes(tmp_path: Path) -> None:
    paths = [
...
def test_p6_delta_manifest_rejects_unsafe_and_duplicate_paths(
    tmp_path: Path,
) -> None:
    path = tmp_path / "file"
...
def test_p6_delta_contains_only_required_stage_a_and_formal_files() -> None:
    assert set(P6_DELTA_FILES) == {
    ...
    assert not any(
        token in relative for relative in P6_DELTA_FILES for token in forbidden
    )


def test_p6_katana_scripts_enforce_stage_and_hardware_gates() -> None:
    paths = (
    ...
    assert "p6_standard_cbm train" not in stage
    formal = paths[2].read_text(encoding="utf-8")
    assert "gpu_model=H200" in formal
    assert "P6_FORMAL_APPROVED" in formal
    assert "p6_standard_cbm train" in formal
    assert "evaluate-test" in formal
    assert "p6_standard_cbm verify" in formal
...
def test_sync_p6_uses_kdm_and_manifest_whitelist(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        ["bash", "scripts/katana/sync_p6_stage_a.sh", str(source)],
        check=False,
        capture_output=True,
        text=True,
        env={
    ...
    assert "p6_standard_cbm.py" in transferred
    assert "p6_stage_a.pbs" in transferred
    assert "p6_stage_a_transfer_manifest.json" in transferred
