...
from lidc_baseline.p8_audit import (
    _alpha_at_best,
    build_oof_audit,
...
def _history(best_epoch: int) -> pd.DataFrame:
    rows = []
    for epoch in range(80):
        row: dict[str, object] = {"epoch_index": epoch}
        for group in CONCEPT_GROUP_ORDER:
            row[f"alpha_{group}_logits"] = json.dumps([0.1, 0, 0, 0, 0])
            row[f"alpha_{group}_weights"] = json.dumps(
                [0.2164807, 0.19587982, 0.19587982, 0.19587982, 0.19587982]
            )
...
def _write_fold(root: Path, fold: int, frame: pd.DataFrame) -> None:
    ...
    completion = {
        ...
        "p8_execution_config_sha256": "3" * 64,
        "split_sha256": str(fold) * 64,
        "encoder_initialization_sha256": str(fold + 1) * 64,
        "combined_gam_initialization_sha256": "4" * 64,
        "best_checkpoint_sha256": "5" * 64,
        "best_alpha_snapshot_sha256": "6" * 64,
        "final_alpha_snapshot_sha256": "7" * 64,
    }
    evaluation = {"status": "TEST_EVALUATED_ONCE", "test_transaction_count": 1}
    ...
    _history(fold).to_csv(run / "history.csv", index=False)
    frame.to_parquet(run / "test_predictions.parquet", index=False)


def test_p8_contribution_reconstruction_and_alpha_audit() -> None:
    ...
    alpha = _alpha_at_best(_history(4), 4)
    assert set(alpha["weights"]) == set(CONCEPT_GROUP_ORDER)
    assert all(len(weights) == 5 for weights in alpha["weights"].values())


def test_p8_build_oof_materializes_private_and_deidentified_outputs(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import lidc_baseline.p8_audit as audit
    ...
    monkeypatch.setattr(
        audit, "verify_fold", lambda fold_index, **_kwargs: verified[fold_index]
    )
    ...
    report = build_oof_audit(
        ...
        p8_execution_config_path=tmp_path / "p8.yaml",
    ...
    assert len(report["learned_alpha_at_best_epoch_by_fold"]) == 5
    ...
    assert "concept_metrics" not in tracked


def test_p8_manifest_mapping_rejects_swapped_patient_keys(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import lidc_baseline.p8_audit as audit
