...
manifest="artifacts/baseline_v2/manifests/p6_stage_a_transfer_manifest.json"
...
if [[ ! -f "$source_directory/$manifest" ]]; then
  echo "Build the P6 Stage A transfer manifest before synchronization." >&2
  exit 2
fi
...
PYTHONPATH="$source_directory/src${PYTHONPATH:+:$PYTHONPATH}" \
  "$python_binary" -m lidc_baseline.p6_katana transfer-list \
