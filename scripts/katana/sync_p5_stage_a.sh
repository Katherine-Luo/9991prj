...
manifest="artifacts/baseline_v2/manifests/p5_stage_a_transfer_manifest.json"
...
if [[ ! -f "$source_directory/$manifest" ]]; then
  echo "Build the P5 Stage A transfer manifest before synchronization." >&2
  exit 2
fi
...
PYTHONPATH="$source_directory/src${PYTHONPATH:+:$PYTHONPATH}" \
  "$python_binary" -m lidc_baseline.p5_katana transfer-list \
