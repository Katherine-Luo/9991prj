...
if [[ ! -f "$source_directory/artifacts/baseline_v2/manifests/p4_transfer_manifest.json" ]]; then
  echo "Build the P4 transfer manifest before synchronization." >&2
  exit 2
fi
...
PYTHONPATH="$source_directory/src${PYTHONPATH:+:$PYTHONPATH}" \
  "$python_binary" -m lidc_baseline.p4_katana transfer-list \
  --repository-root "$source_directory" \
  --transfer-manifest artifacts/baseline_v2/manifests/p4_transfer_manifest.json \
  > "$transfer_list"
"$rsync_binary" -avhP \
