"""Build private P7 OOF predictions and deidentified aggregate evidence."""
...
def _read_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError(f"P7_AUDIT_EXPECTED_OBJECT:{path.name}")
    return payload
...
def contribution_reconstruction_errors(frame: pd.DataFrame) -> dict[str, float]:
    ...
    for group in CONCEPT_GROUP_ORDER:
        ...
        raw += raw_group
        rating += rating_group
        scale_error = np.maximum(
            scale_error, np.abs(rating_group - 4.0 * raw_group)
        )
    raw_error = np.abs(
    ...
    return {
        ...
        "rating_scale_max_absolute_error": float(
            rating_error.max(initial=0.0)
        ),
        "contribution_scale_max_absolute_error": float(
            scale_error.max(initial=0.0)
        ),
...
def _private_run_storage(run: Path) -> dict[str, int]:
    files = [
        ...
        if path.is_file() and not path.name.startswith(".p7_")
    ]
    return {
...
def validate_manifest_patient_mapping(
    ...
    if not required.issubset(manifest.columns):
        raise ValueError("P7_AUDIT_MANIFEST_PATIENT_SCHEMA_MISSING")
    ...
    if (
        ...
        raise ValueError("P7_AUDIT_MANIFEST_PRIMARY_SET_MISMATCH")
    expected = {
    ...
    if set(observed_uids) != set(expected):
        raise ValueError("P7_AUDIT_MANIFEST_OOF_UID_SET_MISMATCH")
    mismatched = [
        uid
        for uid, key in zip(
    ...
    if mismatched:
        raise ValueError("P7_AUDIT_PATIENT_KEY_MAPPING_MISMATCH")
    patient_count = len(set(expected.values()))
    if patient_count != EXPECTED_OOF_PATIENTS:
        raise ValueError("P7_AUDIT_MANIFEST_PATIENT_COUNT_MISMATCH")
...
def _fold_report(
    *,
    ...
    if max(errors.values()) > 1e-6:
        raise ValueError(f"P7_AUDIT_CONTRIBUTION_RECONSTRUCTION_FAILED:{fold_index}")
    if evaluation.get("status") != "TEST_EVALUATED_ONCE":
        raise ValueError(f"P7_AUDIT_TEST_NOT_EXACTLY_ONCE:{fold_index}")
    return {
        "schema_version": SCHEMA_VERSION,
        "status": "PASS",
        "protocol_version": completion["protocol_version"],
        "model": MODEL_NAME,
        "method_label": completion["method_label"],
        ...
        "p7_execution_config_sha256": completion[
            "p7_execution_config_sha256"
        ],
        "split_sha256": completion["split_sha256"],
        "encoder_initialization_sha256": completion[
            "encoder_initialization_sha256"
        ],
        "combined_cem_initialization_sha256": completion[
...
def build_oof_audit(
    *,
    ...
    if fold_counts != EXPECTED_FOLD_TEST_COUNTS:
        raise ValueError("P7_AUDIT_FOLD_TEST_COUNTS_MISMATCH")
    errors = contribution_reconstruction_errors(pooled)
    if max(errors.values()) > 1e-6:
        raise ValueError("P7_AUDIT_POOLED_RECONSTRUCTION_FAILED")
    ...
    report = {
        ...
        "model": MODEL_NAME,
        "method_label": fold_reports[0]["method_label"],
        "folds": 5,
        ...
        "intervention_rates_by_fold": [
            item["intervention_rates"] for item in fold_reports
        ],
        ...
        "p7_execution_config_sha256": fold_reports[0][
            "p7_execution_config_sha256"
        ],
        ...
        "combined_cem_initialization_sha256_by_fold": [
            item["combined_cem_initialization_sha256"] for item in fold_reports
        ],
...
def _parser() -> argparse.ArgumentParser:
    ...
    build.add_argument("--p7-config", type=Path, default=P7_EXECUTION_CONFIG_DEFAULT)
    build.add_argument(
        "--manifest",
        type=Path,
    ...
    build.add_argument("--run-root", type=Path, default=Path("runs/baseline_v2/cem"))
    build.add_argument(
        "--audit-root", type=Path, default=Path("artifacts/baseline_v2/audit/p7")
    )
    build.add_argument(
        "--oof-output",
        type=Path,
        default=Path("runs/baseline_v2/cem/oof_predictions.parquet"),
    )
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_oof_audit(
        ...
        p7_execution_config_path=arguments.p7_config,
